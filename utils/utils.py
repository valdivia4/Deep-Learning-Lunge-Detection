...
def correct_samples(positive_samples, features, correction_model, fs,
    ...
    if corr_model_type == 'regression':
        return correct_samples_regression(positive_samples, features,
                                          correction_model, fs)
    elif corr_model_type == 'classification':
        return correct_samples_class(positive_samples, features,
                                     correction_model, fs)
    else:
...
def get_predictions(
        features, model, flattened_input, correction_model=None, model_type=None, y_pred=None,
        chaining_dist=5, threshold=0.9
):
    skip = 1
    if flattened_input:
...
#     times = np.linspace(int(secs_per_window/2), m*skip + int(secs_per_window/2),m)
    ...
    if correction_model is not None:
        positive_samples = correct_samples(positive_samples, features,correction_model, fs, model_type)
